/**
 * Land cover overlay — ESA WorldCover 2021 Map COGs, AOI-windowed.
 *
 * Same product rules as terrain slope / WDPA: requires a screening area.
 * Caches by AOI bbox (not every pan) so HTTP range reads stay bounded.
 */
...
const SOURCE_ID = "sunday-landcover";
const LAYER_ID = "landcover-raster";
const LAYER_CATALOGUE_ID = "landcover";
...
function previewToDataUrl(p: ViewportPreview): string {
  ...
  if (!ctx) throw new Error("2D canvas unavailable for land cover preview");
...
async function paintLandCover(map: MapLibreMap): Promise<void> {
  ...
  if (!aoi) {
    ...
    if (!guidedMissingAoi) {
      ...
      useUiStore.getState().notify({
        ...
        detail: "Land cover only paints inside a screening area.",
      });
    }
  ...
  if (platform().kind !== "tauri") {
    ...
    useUiStore.getState().notify({
      tone: "info",
      message: "Land cover needs the desktop app",
      detail: "ESA WorldCover tiles are fetched by the native core.",
    });
    useLayerStore.getState().setVisible(LAYER_CATALOGUE_ID, false);
    return;
  }
  ...
  inFlight = (async () => {
    useUiStore.getState().startBusy(BUSY_KEY, "Loading land cover");
    try {
      const next = await platform().landcover.preview(aoi);
      ...
      useUiStore.getState().notify({
        tone: "warning",
        message: "Could not paint land cover",
        detail,
      });
    } finally {
